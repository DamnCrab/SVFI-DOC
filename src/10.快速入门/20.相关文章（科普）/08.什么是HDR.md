...
* **辐射通量**：在光辐射测量学中，使用的基本物理量是`辐射通量`或`辐射功率`，符号表示为：$\Phi_e$​，单位是瓦特（W）。辐射通量由一个辐射源发出，并通过某种传播介质进行传输并在某种表面上接收该辐射通量。

* **光通量**：`光通量`是指按照国际规定的标准人眼视觉特性评价的辐射通量的导出量，也就是对`辐射通量`的光度量，其符号为：$\Phi_v$​。可以从`辐射通量`来导出`光通量`。

* **光量**： `光量`也称为`光能`，指的是在给定的时间间隔（$\Delta t$）内，`光通量`的时间积分，符号表示为：$Q_v$​，且$Qv\backslash =∫ΔtΦv⋅dtQ\_\{v\}=\\int\_\{\Delta t\}\{\Phi\_\{v\} \cdot dt\}Qv\backslash =∫Δt​Φv​⋅dt$。
