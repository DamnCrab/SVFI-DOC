...
也可以不依赖外部 TC：关闭自动转场识别后，用软件自带的离线检测生成 JSON，再在[内置播放器](/zh/pages/player/#转场检测结果确认)里微调。

1. 关闭**开启转场识别**，在转场设置中点击 **执行自动转场检测**，对当前输入视频做一次离线识别。完成后 JSON 路径会自动填入。
2. 在任务列表中右键该条目，选择 **开启内部播放器来切分任务段**（也可直接打开播放器）。播放器用法见[播放器介绍](/zh/pages/player/)。

<div align=center>
<img src="/screenshots/zh/22-auto-scene-detect-A.png"  width=600>
</div>

3. 打开播放器后，切到 **转场确认**。时间轴上的标记是检测结果。详见[播放器介绍](/zh/pages/player/#转场检测结果确认)。
