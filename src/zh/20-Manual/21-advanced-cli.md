...
`SVFI-Skills` 是随已安装 SVFI 根目录分发的便携式 Codex/Cursor 技能包。请在该安装目录下的 `SVFI-Skills` 子目录执行命令，并使用与当前安装版匹配的技能版本。不要用源码树的 `python one_line_shot_args.py` 去跑安装版 OLS。

### 安装命令

在安装版的 `SVFI-Skills` 目录中执行：
...
```powershell
python .\install.py --dest "D:\CodexSkills" --skills svfi-environment-init svfi-ols-task-run
```

不带 `--dest` 时，安装器将目标设为 `$env:CODEX_HOME\skills`；若 `CODEX_HOME` 未设置，则回退到 `%USERPROFILE%\.codex\skills`。因此上述全量命令默认写入当前用户的 Codex 个人 skills 目录，而不是 SVFI 安装目录。安装到 Cursor 时使用 `--dest "%USERPROFILE%\.cursor\skills"`。指定 `--dest` 可改用其他 skills 目录；目标已存在时，使用支持的 `--force` 才会替换所选技能目录。

随包的技能名和用途如下：

| 技能 | 用途 |
|---|---|
| `svfi-environment-init` | 初始化已安装的 SVFI 根目录，写入 `steam_appid.txt`，并生成/运行带引号的帮助批处理文件。 |
| `svfi-ols-task-run` | 为 VFI、仅渲染、拼接、抽取和管道模式创建并运行带引号的 OLS 任务批处理文件。 |

### 本地 OLS 用法

- 路径含空格时必须先生成带引号的 `.bat`，再用 `cmd.exe /d /c` 运行；不要把未加引号的空格路径直接塞进 `cmd /c`。
- 目标可执行文件是安装根下的 `one_line_shot_args.exe`，不要用源码树 Python 驱动安装版。
- 用 `--force-args key=value` 覆盖导出的 `.ini`/`.json`，不必改配置文件本身。
- 正式任务前保持 Steam 桌面客户端已登录。日志出现 `SteamNotLoadedException` 或 `Failed to initiate Steam API` 属于预检失败：重启 Steam 后重跑同一批处理，不要靠改任务参数绕过。

### 重启或新建 task 后调用

安装完成后重启 Codex，或新建一个 Codex task，使个人 skills 目录重新加载。**执行 OLS 任务的新 task 先调用 `$svfi-environment-init`，确认环境初始化成功后再调用 `$svfi-ols-task-run`；查询配置快照时可直接调用 `$svfi-config-build`，无需先初始化。**例如：

```text
请调用 `$svfi-environment-init`，初始化当前 SVFI 安装根目录并报告结果。
```

执行 OLS 任务时，环境初始化完成后调用：

```text
请调用 `$svfi-ols-task-run`，按当前任务参数创建并运行 OLS 任务批处理文件。
```

查询便携配置快照时可直接调用：

```text
请调用 `$svfi-config-build`，核对当前安装版的 CLI、配置键和 GAS 清单。
```
