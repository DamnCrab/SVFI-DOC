...
- **前进消重**: 完全去除动漫视频素材的卡顿，仅支持选择补帧倍率(2x,3x,4x,...)，如果您输入的视频帧率在24左右，默认填2即可，填0则会自定决定 **( 动漫 ) ( 强烈推荐漫剪使用 )**
- **平滑差异**: 当视频出现不规律的重复帧（不确定多少帧一重复），或者处理录屏掉帧视频时，可以使用该功能进行一次平滑（注意这不会改变输入视频的帧率），值可以开5，10，15，20，...等自行尝试（对于不同素材值不一）
