...
`SVFI-Skills` is a portable Codex/Cursor skill bundle distributed with an installed SVFI root. Run the commands from the `SVFI-Skills` subdirectory of that installation and use the skill version matched to the installed build. Do not drive installed OLS with source-tree `python one_line_shot_args.py`.

### Installation commands

Run the following from the installed bundle's `SVFI-Skills` directory:
...
Without `--dest`, the installer resolves the destination to `$env:CODEX_HOME\skills`; when `CODEX_HOME` is unset, it falls back to `%USERPROFILE%\.codex\skills`. Thus the all-skills command above writes to the current user's Codex personal skills directory, not the SVFI installation directory. For Cursor, pass `--dest "%USERPROFILE%\.cursor\skills"`. Use `--dest` for another skills directory; when a destination already exists, the supported `--force` option replaces the selected skill directories.

The bundled skill names and purposes are:

| Skill | Purpose |
|---|---|
| `svfi-environment-init` | Prepare an installed SVFI root, write `steam_appid.txt`, and generate/run a quoted help batch file. |
| `svfi-ols-task-run` | Create and run quoted OLS task batch files for VFI, render-only, concat, extract, and pipe modes. |

### Local OLS usage

- When any path contains spaces, generate a quoted `.bat` first and run it with `cmd.exe /d /c`; do not pass unquoted space-containing paths through `cmd /c`.
- Target `one_line_shot_args.exe` in the install root. Never use source-tree Python to drive an installed product.
- Override an exported `.ini`/`.json` with `--force-args key=value` instead of editing that file.
- Before a real task, keep the Steam desktop client signed in. `SteamNotLoadedException` / `Failed to initiate Steam API` is a preflight failure: restart Steam and rerun the same batch; do not rewrite task parameters to paper over it.

### Calling the skills after a restart or new task

After installation, restart Codex or create a new Codex task so the personal skills directory is loaded again. **In a new task that runs an OLS job, call `$svfi-environment-init` first, confirm that initialization succeeded, and then call `$svfi-ols-task-run`; a configuration-snapshot query can call `$svfi-config-build` directly without initialization.** For example:

```text
Please call `$svfi-environment-init` to initialize the current SVFI installation root and report the result.
```

For an OLS job, call the task skill after initialization:

```text
Please call `$svfi-ols-task-run` to create and run an OLS task batch file using the current task parameters.
```

For a portable configuration-snapshot query, call the config skill directly:

```text
Please call `$svfi-config-build` to check the CLI, config-key, and GAS inventories for the installed build.
```
